...
- **Path 1 (curated feed):** `fetch.Table(ctx, fetch.WithURL(...))` downloads a pre-compiled TGEO binary from any URL serving a gzipped TGEO file. One function call, automatic HTTP conditional request caching.
...
`WithURL` specifies the feed URL serving a gzipped TGEO binary. Feed operators can also publish a `meta.json` alongside the binary, matching the `tgeo.Meta` struct, so consumers can check the current version and checksum without downloading the full file.
